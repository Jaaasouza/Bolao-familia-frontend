// Pool chat. Logged-in participants post and read messages; the list polls every
// few seconds and auto-scrolls. Admins get a small delete control per message.
...
export default function ChatView() {
  ...
  // Auto-scroll to the newest message, but only if the user is already at the
  // bottom (don't yank them up while they're reading history).
  useEffect(() => {
  ...
  const send = async (e) => {
    ...
    try {
      const res = await API.chatPost(body.slice(0, MAX_LEN), token);
  ...
  if (!token) {
    return (
      <div className="card">
        <h2>{t('chatTitle')}</h2>
        <p className="hint">{t('chatSignIn')}</p>
      </div>
    );
  }
